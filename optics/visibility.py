...
ds['pl_watt'] = ds['pl_watt_flux'] * 4*pi * ds['pl_rade']**2
ds['pl_phps'] = ds['pl_watt'] / photon_energy(10E-6)
ds['st_watt_flux'] = blackbody_flux(ds['st_teff'])
ds['st_watt'] = ds['st_watt_flux'] * 4*pi * ds['st_rad']**2
ds['st_phps'] = ds['st_watt'] / photon_energy(10E-6)

# Measured fluxes
# TODO: Aperture effective area
ds['pl_meas_phps'] = ds['pl_phps'] * (1/2) / (4*pi * ds['sy_dist']**2)
ds['st_angdia'] = ds['st_rad']/ds['sy_dist']
ds['st_meas_phps'] = ds['st_phps'] * \
    null_depth(ds['st_angdia']) / (4*pi * ds['sy_dist']**2)

# SNR
ds['sh_noise'] = shot_noise(ds['pl_meas_phps'], ds['st_meas_phps'])
ds['t_int'] = (5 * ds['sh_noise'] / ds['pl_meas_phps'])**2
ds.loc[(ds['visibility'] == Visibility.VISIBLE) & (
    ds['t_int'] > 60*60*30), 'visibility'] = Visibility.SNR_TOO_LOW
